...
function fieldToZod(field: FieldSchema) {
  ...
  switch (field.type) {
    ...
    case 'number': {
      ...
      if (v?.required) schema = schema.refine((val) => val !== null && val !== undefined, {
    ...
    case 'checkbox': {
      ...
      if (v?.required) schema = schema.refine(Boolean, { message: typeof v.required === 'string' ? v.required : 'Required' });
      return schema;
    }
